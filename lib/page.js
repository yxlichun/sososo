...
var config = require('../util/config').getConfig().getBasicConfig();

var updateAttachedContent = require('./updateAttachedContent')
var fileName = require('../util/fileName')

function run (template, moduleName, pageName) {
  var to = path.join(config.destBasePath, fileName.getHeadUpper(pageName));

  var metalsmith = Metalsmith(path.join(config.templatePath, '/src/containers', template));
